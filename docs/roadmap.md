...
**Решения по объёму (согласовано с автором).**
- Текст ошибки прохождения формы — **оставляем текущие формулировки**
  (отправка → серверный текст или fallback «Не удалось отправить
  отклик. Попробуйте ещё раз.»; загрузка 404 → «Форма не найдена»;
  прочая загрузка → «Не удалось загрузить форму. Попробуйте обновить
  страницу.»). Требование «согласовать текст» закрыто.
- В объём добавлены **чистка техдолга** этапов 10–11 и **визуальная
  полировка layout** (сначала аудит макеты↔реализация → согласование
  списка расхождений → правки).
- **Flash-сообщения auth** («Для просмотра…» / «Вы уже вошли…») —
  **вне объёма**, остаются отложенным долгом этапа 10.
- Toast реализуем **кастомно на `Alert`** (Zustand-стор + viewport),
  без новой npm-зависимости.

**Подзадачи (коммиты).**
1. `docs`: roadmap — подзадачи этапа 12.
2. `shared/ui`: toast — `useToastStore` (очередь + авто-таймаут) +
   `ToastViewport` на `Alert` (`role="status"`) + barrel + тест стора.
3. `app`: смонтировать `<ToastViewport />` в `AppProviders`.
4. `features/copy-form-link`: toast «Ссылка скопирована» при успешном
   копировании (+ сообщение об ошибке) + сценарный тест.
5. `widgets/form-fill`: a11y — `aria-invalid`/`aria-describedby` на
   `ShortText`/`LongText`/`ChoiceQuestion` со связью на ошибку
   валидации, `aria-live` на экране успеха + сценарные тесты.
6. `widgets/question-card`: кнопки «вверх/вниз» — нативный `disabled`
   вместо `aria-disabled` + тест.
7. `entities/user`: убрать неиспользуемые RQ-хуки (`useCreateUser`/
   `useUpdateUser`/`useFindUserByEmail`), оставить функции + `userKeys`.
8. `features/logout`: `LogoutButton` на shadcn `Button` (`variant="ghost"`)
   + обновить тест.
9. `shared/lib`: `getInitials` + unit-тест, подключить в `ProfilePage`.
10. `pages/profile`: `return null` → `<Navigate to={routes.login} replace />`.
11. `test`: сквозной интеграционный сценарий — auth (логин/регистрация/
    редирект анонима).
12. `test`: сквозной интеграционный сценарий — форма (создать → пройти →
    увидеть отклик), покрывает `NewFormPage`.
13. Визуальная полировка — по согласованному после аудита списку
    (отдельные коммиты на страницу/группу правок).
